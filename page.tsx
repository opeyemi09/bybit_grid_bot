...
const API_BASE_URL = "https://d326-197-211-59-121.ngrok-free.app"
const FALLBACK_API_BASE_URL = "http://localhost:3001" // Fallback to local development server if ngrok fails
...
export default function TradingBotDashboard() {
  ...
  const fetchData = async () => {
    try {
      ...
      let tradesResponse = null
      let currentApiUrl = API_BASE_URL

      try {
        // Try the primary API URL
        ;[statusResponse, tradesResponse] = await Promise.all([
        ...
        console.error("Primary API URL failed:", primaryError)

        // If primary URL fails, try fallback URL
        try {
          console.log("Trying fallback API URL...")
          currentApiUrl = FALLBACK_API_BASE_URL[(statusResponse, tradesResponse)] = await Promise.all([
            fetch(`${FALLBACK_API_BASE_URL}/api/status`, {
              headers: {
                "ngrok-skip-browser-warning": "true",
              },
              signal: AbortSignal.timeout(5000),
            }),
            fetch(`${FALLBACK_API_BASE_URL}/api/trades?limit=50`, {
              headers: {
                "ngrok-skip-browser-warning": "true",
              },
              signal: AbortSignal.timeout(5000),
          ...
          console.error("Fallback API URL also failed:", fallbackError)
          throw new Error(`API unreachable: ${primaryError.message}. Fallback also failed: ${fallbackError.message}`)
        }
      }

      if (!statusResponse?.ok) {
        const errorText = statusResponse ? await statusResponse.text().catch(() => "Unknown error") : "Network error"
        throw new Error(`Status API returned ${statusResponse?.status || "no response"}: ${errorText}`)
      }

      if (!tradesResponse?.ok) {
        const errorText = tradesResponse ? await tradesResponse.text().catch(() => "Unknown error") : "Network error"
        throw new Error(`Trades API returned ${tradesResponse?.status || "no response"}: ${errorText}`)
      }

      const statusData = await statusResponse.json()
      const tradesData = await tradesResponse.json()

      console.log(`API connection successful using ${currentApiUrl}!`)
      console.log("Status data:", statusData)

      // Transform the enhanced API response to match our expected structure
      ...
      // Fetch additional data using the successful API URL
      fetchGridData(currentApiUrl)
      fetchPerformanceData(currentApiUrl)
      fetchMarketData(currentApiUrl)
    } catch (err) {
  ...
  const sendControlCommand = async (action: string) => {
    try {
      ...
      let response = null
      try {
        response = await fetch(`${API_BASE_URL}/api/control`, {
          method: "POST",
          headers: {
        ...
        console.error("Primary API control command failed:", primaryError)

        // Try fallback URL
        response = await fetch(`${FALLBACK_API_BASE_URL}/api/control`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            "ngrok-skip-browser-warning": "true",
  ...
  const fetchGridData = async (apiUrl = API_BASE_URL) => {
    try {
      const response = await fetch(`${apiUrl}/api/grid`, {
        headers: {
          "ngrok-skip-browser-warning": "true",
        },
        signal: AbortSignal.timeout(5000),
      ...
      // Update botData with grid information
      setBotData((prev) =>
        prev
          ? {
              ...prev,
  ...
  const fetchPerformanceData = async (apiUrl = API_BASE_URL) => {
    try {
      ...
      // Update botData with performance information
      setBotData((prev) =>
        prev
          ? {
              ...prev,
  ...
  const fetchMarketData = async (apiUrl = API_BASE_URL) => {
    try {
      ...
      // Update botData with market information
      setBotData((prev) =>
        prev
          ? {
              ...prev,
  ...
  useEffect(() => {
    ...
    // Update data periodically with exponential backoff for retries
    const interval = setInterval(() => {
      // If connected, use normal interval
      // If disconnected, use exponential backoff based on retry count
