...
app.get("/api/market", async (req, res) => {
  try {
    // Price history, current/previous prices
    const ph = gridBot.priceHistory || [];
    const current = ph.length > 0 ? ph[ph.length - 1] : 0;
    const previous = ph.length > 1 ? ph[ph.length - 2] : 0;
    const history = [...ph];
    const changePercentage = previous ? ((current - previous) / previous) * 100 : 0;

    // ATR, Volatility, Market Condition
    ...
    res.status(500).json({ error: "Failed to get market data", details: err.message });
  }
});

// GET live Bybit spot orders (open orders)
app.get("/api/live-orders", async (req, res) => {
  try {
    const response = await gridBot.bybitClient.getActiveOrders({
      category: "spot",
      symbol: CONFIG.SYMBOL,
    });
    if (response.retCode === 0) {
      console.log(res)
      res.json({ orders: response.result.list || [], total: (response.result.list || []).length });
    } else {
      res.status(500).json({ error: response.retMsg });
    }
  } catch (e) {
